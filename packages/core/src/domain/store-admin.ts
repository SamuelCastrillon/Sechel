...
export async function doctorDiagnostics(
  ...
  const [countObs, countSessions, countPrompts, countRelations, countPending, countOrphaned, maxCreated] =
    await Promise.all([
      sql<{ c: number }>`SELECT COUNT(*) AS c FROM observations WHERE tenant_id = ${tenantId}`.execute(db),
      ...
      sql<{ c: number }>`SELECT COUNT(*) AS c FROM memory_relations WHERE tenant_id = ${tenantId}`.execute(db),
      sql<{ c: number }>`SELECT COUNT(*) AS c FROM memory_relations WHERE tenant_id = ${tenantId} AND judgment_status = 'pending'`.execute(db),
      sql<{ c: number }>`SELECT COUNT(*) AS c FROM memory_relations WHERE tenant_id = ${tenantId} AND judgment_status = 'orphaned'`.execute(db),
      sql<{ created_at: string | null }>`SELECT MAX(created_at) AS created_at FROM observations WHERE tenant_id = ${tenantId}`.execute(db),
    ]);
...
export async function capturePassive(
  ...
  for (const item of items) {
    ...
    const saved = await saveObservation(db, tenantId, actor, {
      title,
      content: item,
      type: 'learning',
  ...
  return { saved: observations.length, skipped, observations };
}
